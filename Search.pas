...
//+++++++++++++++Node in search tree++++++++++++++++++++++++++++++++++++++++++++
  Node = record
    ...
    nodenum: Int64;//Anzahl der Z�ge, um diese Tiefe zu vollenden
    {$IF QTM}
    virtualmove: Boolean;//true wenn in Phase2 in Zug R,F,L,B,Rs,Fs ausgef�hrt wird
    {$IFEND}
  end;
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
begin
  ...
  maxLength:=maxLen;
  np:= @n[depth];
  r_depth:=0; //r_depth z�hlt die noch zu besetzenden freien Felder rechts (mit wachsendem Index)
...
{$IF  UHUGE} //UltraHuge Solver
  if USES_BIG and not isOriented then   //Ultrabig Solver  f�r orientierte Cubes lansamer als Standard
  begin
   ...
   if isOriented then
    ...
    begin
 {$IF QTM}
      if (not slicemode) and not (Odd(r_depth) xor np1^.parityEven) then goto incAxis;//im slicemode �ndern manche Z�ge nicht die Parit�t der Ecken!
 {$IFEND}

//Es kann gefolgert werden, dass der Pruningwert eins h�her ist, wenn alle gleich sind.
  ...
  {$ELSE} //Huge Solver
  if USES_BIG and not isOriented then //auch nicht f�r oriented cubes
  begin
    np1^.UDTwist:= TwistMove[np^.UDTwist,m];
...
//Wert eins h�her ist, wenn alle gleich sind.
      if (r_depth>0) and (np1^.UDPrunBig=np1^.RLPrunBig)
                     and (np1^.RLPrunBig=np1^.FBPrunBig) then
      begin
      {$IF not QTM}
  ...
  else//standard optimal solver, l�st auch oriented cubes
  begin

    if isOriented then
    begin

      np1^.UDSliceSorted:= UDSliceSortedMove[np^.UDSliceSorted,m];//f�r das Pruning aus UDSliceSorted und UDcenTwist 11880*4096
      np1^.UDcenTwist:= CentOriMove[np^.UDcenTwist,m];

      np1^.UDTwist:= TwistMove[np^.UDTwist,m];
      np1^.UDCentRFLBMod2Twist:= CentOriRFLBMod2Move[np^.UDCentRFLBMod2Twist,m];

      x:= flipSliceMove[np^.UDIdx,Ord(SymMove[np^.UDSym,m])];//wird f�r das Pruning aus flipslice, twist und centrflbMod2 gebraucht
      np1^.UDSym:=SymMult[x and 15,np^.UDSym];
      np1^.UDIdx:= x shr 4;
...
//Wert eins h�her ist, wenn alle gleich sind.
  ...
  begin
    ...
    begin
      ...
      np1:=np;
//zun�chst mal die Bilder berechnen
      for j:=fixNum downto 0 do Dec(np1);

      //Z�ge werden verworfen, wenn die lex. Ordnung einer Transformation kleiner ist.
      //Die Reihenfolge f�r parallele Achsen kann durch die  Transf. vertauscht sein, das erh�ht aber
      //immer die lex. Ordnung. Es kann also h�chstens passieren, dass ein Zug nicht
      //verworfen wird, obwohl es m�glich w�re.
      ...
      //sonst w�re er ja schon vorher erkannt worden.
      ...
      //im n�chsten Zug sowieso erh�ht.
      ...
      if  m1 < m then                                                 ///////////////////////////////////////////////
        goto  incAxis  //kleiner, verwerfen
             else if m1 > m then break;//gr��er
        //bei Gleichheit n�chste Stelle untersuchen
      end;
    end;//i
  end;//if sym>1
//Ende Symmetriecheck
  ...
  {$IF SPECIAL4}
    if depth>12 then //bei 13 ist maximale L�nge 14
    begin
      returnLength:=0;
      Result:=returnLength; //abort
      goto ende;
...
begin
  ...
  if isOriented then //dann wird im Augenblick nur der Standardsolver verwendet, da die gro�en nichts bringen
  begin
    for i:=invalid to depth do
    begin
      m:= Move(3*Ord(n[i].axis) + n[i].power - 1);

 //     n[i+1].UDcenTwist:= CentOriMove[n[i].UDcenTwist,m]; //beim Standard Solver nicht n�tig

      n[i+1].cornPos:=CornPermMove[n[i].cornPos,m];

 //     n[i+1].UDSliceSorted:=UDSliceSortedMove[n[i].UDSliceSorted,m]; //wird beim Standardsolver mit Orientierung mitgef�hrt
 //     m:= SymMove[16,m];
 ...
 //     n[i+1].FBSliceSorted:=UDSliceSortedMove[n[i].FBSliceSorted,m];
  ...
  begin
    ...
    begin
      ...
      n[i+1].UDSliceSorted:=UDSliceSortedMove[n[i].UDSliceSorted,m]; //N�tig bei Standardsolver ohne Orientierung und beim UHUGE solver
      m:= SymMove[16,m];
      n[i+1].RLSliceSorted:=UDSliceSortedMove[n[i].RLSliceSorted,m];
      m:= SymMove[16,m];
      n[i+1].FBSliceSorted:=UDSliceSortedMove[n[i].FBSliceSorted,m];
...
//generate phase2 coordinates
 ...
 inValid:=depth;
//bei orientierten W�rfeln muss die Parit�t der UDSlice*2 mit der Summe der RFLB
//centertwists mod4 �bereinstimmen!
 if isOriented then
 begin
...
left:
  ...
  begin
    ...
    if isOriented and (depth2>9) then  //ignore solutions with phase2length>10  !!!!!!f�r phase2 Demos auf 30 erh�hen!!!!!!!!!!!!!
    begin
...
//precheck only with corners
 ...
 if temp> maxLength-(depth+1)+1 then begin depth2:=-2; goto ende; end;//no solution, new power, da von U nach �' sich die L�nge in Phase 1 um 2 ver�ndern kann
 if temp> maxLength-(depth+1) then begin depth2:=-2; goto ende; end;//no solution, new power
...
// np:= @n[depth+1];
 ...
 if isOriented and (np^.UDCenTwist<>0) then goto xxx;  //Keine Identit�t
...
// temp:= PruningCenTwistUDSliceSorted[np^.UDSliceSorted shl 12 +np^.UDcenTwist];
 ...
 if temp> maxLength-(depth+1)+2 then begin depth2:=-3; goto ende; end;//no solution, new axis
 if temp> maxLength-(depth+1)+1 then begin depth2:=-2; goto ende; end;//no solution, new power, da von U nach U' sich die L�nge in Phase 1 um 2 ver�ndern kann
 if temp> maxLength-(depth+1) then begin depth2:=-2; goto ende; end;//no solution, new power


 if not slicemode then// im Us,Rs,Fs �ndern die Parit�t nicht
 begin
...
//  np1:=np; Dec(np1);
//  if {(depth<>r_depth){have left neigbour and}(np1^.axis=np^.axis)//linker Nachbar ist immer vorhanden, da Phase 1 immer eine L�sung hat
...
turn: //nicht bei virtual moves
  ...
  begin
    if r_depth=0 then goto incAxis;//keine L�sung mit virtuellem Zug am Ende  falsch, nur neue Achse!!!
    np1^.cornPos:= np^.cornPos; //Koordinaten einfach �bertragen
  ...
  symCPos:= CornPosToSymCornPos[np1^.cornPos];
  _sym:= symCPos and $f;
  symCPos:=symCPos shr 4;//die �quivalenzklasse
...
//  np1^.UDPrun:= GetPruningLength[np^.UDPrun,GetPruningPhase2P(2768*Edge8PosConjugate[np1^.edge8Pos,_sym] + symCPos)];
  ...
  if (np1^.UDPrun>r_depth+2) then goto incAxis; //kommt bei RR FF LL unb BB h�ufig vor!
  if (np1^.UDPrun>r_depth) then goto incPower;
...
begin
 ...
 //jetzt f�r den Fall useSlices das Maneuver berechnen
 ...
 begin
   ...
   if invertPower then p:=4-p;
     s1:=s1+numToStr(p);
 //jetzt den W�rfel entsprechend drehen.
...
begin
  ...
  {$ELSE}
  until returnlength<=stopAt+17; //Dr�ber wird keine L�sung angezeigt!!
  {$IFEND}
end;
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
