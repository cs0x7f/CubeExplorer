...
procedure CreateGetCorn8PermTable;//f�r den Coset-Explorer
...
//++++++++++++++++Arrays for the Conjugation by symmetries++++++++++++++++++++++
    ...
    UDSliceSortedConjugate: array[0..24-1,0..15] of Word;//eingeschr�nkt auf phase2!
...
//++++++++++++++++++++++++++F�r den Coset-Explorer++++++++++++++++++++++++++++++
   BitVector: array {[0..24*40320*288/32-1]} of Integer;//f�r den Bitvektor 24*40320*288
   BitVectorPS: array {[0..24*40320*288/32-1]} of Integer;//f�r den Bitvektor 24*40320*288
   GetCorn8Perm: array[0..40320-1] of Word;//Eckenpermutationen in den Tetraedern

//++++++++++++++++++++++++++F�r phase 2 bei orientierten Cubes++++++++++++++++++
...
begin
  ...
  begin
    ...
    //testen ob die ersten vier Koordinaten �berhaupt im Tetraeder liegen
...
//ich habe ein besseres Gef�hl, das auf die Position von zwei Steinen zu beziehen.
...
begin
  ...
  CreateGetCorn8PermTable;//f�r Coset-Explorer
//  CreateCorn8PermMoveTable;  //f�r den Coset Explorer (Prescan)
  ...
  begin
   ...
   //   PruningPhase2Q[0]:= PruningPhase2Q[0] or (13 shl (4 ));
    ...
    begin
      ...
      for i:=0 to 2768*40320-1 do  //hier erst mal nur mit den Drehungen der L�nge 1
      begin
        // if i and $3ffff =0 then Application.ProcessMessages;
        ...
        begin
          ...
          symCornPos0:= i mod 2768;
          cornPos0:=SymCornPosToCornPos[symCornPos0];
          for m:= Ux1 to Bx3 do//Usx3 do//slice mode funktioniert nicht mit Komprimierung, da Usx etc die Parit�t nicht �ndert
          begin
            if (not sliceMode) and (m=Usx1) then break;
            case m of
               Ux2,Rx1,Rx2,Rx3,Fx1,Fx2,Fx3,Dx2,Lx1,Lx2,Lx3,Bx1,Bx2,Bx3,Usx2:continue;//only Ux1,Ux3,Dx1,Dx3,Usx1,Usx3
    ...
    if depth>0 then
    begin
      for i:=0 to 2768*40320-1 do  //hier mit den Drehungen der L�nge 2
      begin
        // if i and $3ffff =0 then Application.ProcessMessages;
    ...
    for i:= 0 to 2768-1 do
      depthmod2Remain[i]:= PruningPhase2Q[i] mod 2;//ob gerade oder ungerade Tiefe h�ngt nur von der symcornpos ab
...
{$IFEND}
  ...
  begin
    ...
    // Form1.ProgressBar.Position:=90;//evtl �ndern!!!
    fs.Free;
  end
  else
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          for m:= Ux1 to Fsx3 do //slice moves erg�nzt
          begin
...
{$IFEND}
    ...
    buf: array[0..2187-1] of Byte;  // Chunks der L�nge 2187 Byte
    filename:String;
...
{$IFEND}
 ...
 //Finalize(PruningUBigP);
  ...
  begin
    ...
    try
      for i:= 0 to 69 do
         SetLength(PruningUBig[i],8806776); //2187*64430 div 16 +1 f�r alle 70 tetraCoords;
    except
      // Application.MessageBox(PChar(Err[17]),'',MB_ICONWARNING);
      // Form1.ProgressLabel.Visible:=false;
      // Form1.Progressbar.Visible:=false;
            ...
            case flipBackward of
              ...
              begin
                ...
                begin
                  ...
                  Inc(done);
                  goto NextI;
                 // break;//verl�sst nur m-Schleife, eigentlich kann man aber
                end;    //das n�chste i aufrufen!
              end;
            end;
          end;//for m
        end;//if match
  ...
  //  Form1.ProgressBar.Position:=12;
    ...
    try
       for i:= 0 to 69 do
         SetLength(PruningUBigP[i],64430*2187 div 5); //2187*64430/5 f�r alle 70 tetraCoords;
    except
      // Application.MessageBox(PChar(Err[17]),'',MB_ICONWARNING);
      // Form1.ProgressLabel.Visible:=false;
      // Form1.Progressbar.Visible:=false;
