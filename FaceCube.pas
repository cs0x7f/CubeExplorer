...
//Sonderbehandlung f�r Z�ge, die die Mitten ver�ndern
  else if _x=E then
  begin
...
begin
  ...
  // //size:=4; //F�r das zeichnen von Grafiken hier patchen 4 oder 7 nehmen
  // d:=size;
  // for i:=U1 to B9 do
  // begin
...
//pr�ft, wie oft eine bestimmte Ecke bei der aktuellen Farbbelegung prinzipiell vorkommen kann
begin
...
begin
  ...
  if auto and (fc < B9) then exit;//fc wird als letztes �berpr�ft, wenn alle gecheckt werden


//  hier muss getestet werden, ob es eine valide Konstruktion ist!!!!

//1. es d�rfen keine nicht vollst�ndigen Ecken und Kanten �brigbleiben um weiterzumachen
   for k:= URF to DRB do
   begin
     m:=0;t:=0;
     for n:=0 to 2 do
   ...
   begin
    ...
    if n>=2 then begin cCube.Free; exit end;//bei zwei freien Pl�tzen keine Parity Probleme
    n:=0;
...
//onlyNormal: Es werden nur die Symmetrien genommen, in deren Gruppe die ausgew�hlte Symmetriegruppe
//normal ist, d.h. x^-1Hx=H f�r alle in Frage kommenden Symmetrien x.
//alsoInverse f�hrt die Untersuchung auch f�r inversen Cubes durch
...
begin
         ...
         begin
           ...
           begin
             if onlyCorners then
              ...
              if PFace^[i]<>f.PFace^[i] then begin isEqual:=false;break;end;//n�chste Symmetrie nehmen
           end;
 ...
 //          Result:=false; goto ende;//Patch, wenn nur Identische gepr�ft werden sollen
weiter:    Inc(index);
           Conjugate(S_LR2);
         end;
         Conjugate(S_U4);
...
ende:
  ...
  if alsoInverse and (Result=false) then //noch bzgl. der Inversen pr�fen
  begin
